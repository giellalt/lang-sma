...
grep ";" noun-sma-lex.txt | grep -v "^\!" | grep ' R '| cut -d ":" -f1 | sed 's/+Use\/Sub//' | sed 's/+Dial\/-SW//' | sed 's/+Dial\/-NOR//' | sed 's/+CmpN\/Pref//' | sed 's/CmpN\/SgN$/Cmp#gåetie+N+Sg+Nom/' |  sed 's/CmpN\/PlG$/Cmp#gåetie+N+Sg+Nom/' | $LOOKUP $GTHOME/gt/sma/bin/isma.fst | cut -f2 | grep "\-" | grep -v "\-.*\-" | cut -d "-" -f1 > Ranalnouns
grep ";" noun-sma-lex.txt | grep -v "^\!" | grep ' R '| cut -d ":" -f1 | sed 's/+Use\/Sub//' | sed 's/+Dial\/-SW//' | sed 's/+Dial\/-NOR//' | sed 's/+CmpN\/Pref//' | sed 's/CmpN\/SgN$/Cmp#gåetie+N+Sg+Nom/' |  sed 's/CmpN\/PlG$/Cmp#gåetie+N+Sg+Nom/' | $LOOKUP $GTHOME/gt/sma/bin/isma.fst | cut -f2 | grep "\-" | grep "\-.*\-" | cut -d "-" -f1,2 >> Ranalnouns
