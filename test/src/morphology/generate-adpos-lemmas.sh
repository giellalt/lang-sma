#!/bin/bash
# test/src/morphology/generate-adpos-lemmas.sh.  Generated from generate-adpos-lemmas.sh.in by configure.
...
# - an exit status of 99 a hard errorg
...
# Sorter og unifiser
        sort -u -o $generated_lemmas.$f.tmp $generated_lemmas.$f.tmp

# Sammenlikne lista med adposlemmaer med den genererte lista
		comm -23 $lemmas $generated_lemmas.$f.tmp > $result_file.$f.txt
