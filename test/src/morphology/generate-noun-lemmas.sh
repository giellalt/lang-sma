...
/Users/mka055/langtech/GitHub/giellalt/lang-sma/./../giella-core/scripts/extract-lemmas.sh \
	--exclude "(CmpN/Only| R | Rreal |\+Gen\+|\+Der\+|\+CmpN\/Suff|\+Use\/MT)" \
	$source_file > $lemmas
...
#grep ' R.*€' $source_file  | cut -d '€' -f2 > testcmplemma.txt
#grep ' R.*€' $source_file  | cut -d '+' -f1 > $comp_lemmas
...
###### Test non-comopunds: #######
        ...
        # Finally, try the remaining nouns as TODO entries:
        sed 's/$/+N+Sg+Nom/' $lemmas | $lookup_tool $generator_file.$suffix \
            | fgrep "+?" | cut -d "+" -f1 | sed 's/$/+N+Pl+Nom/' \
            | $lookup_tool $generator_file.$suffix \
            | fgrep "+?" | cut -d "+" -f1 | sed 's/$/+N+TODO/' \
            | $lookup_tool $generator_file.$suffix | cut -f2 \
            | grep -v "^$" >> $generated_lemmas.$f.txt 

###### Test compounds: #######
		# Test nouns that only can generate as compounds:
#		sed 's/$/+N+Cmp\/SgNom+Cmp\/Hyph+Cmp#gåetie+N+Sg+Nom/' $comp_lemmas \
#			| $lookup_tool $generator_file.$suffix \
#			| grep "\-" | cut -f2| cut -d "-" -f1 > $generated_comp_lemmas.$f.txt
...
#        comm -23 $comp_lemmas $generated_lemmas.$f.txt \
#        comm -23 testcmplemma.txt $generated_comp_lemmas.$f.txt \
                >> $result_file.$f.txt
