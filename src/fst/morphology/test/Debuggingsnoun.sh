...
grep ";" ../../../src/morphology/stems/nouns.lexc | egrep -v "(^\!|\+Err\/Lex)" | egrep -v '(\+Gen\+|\+Der\+| R )' | sed 's/% /€/g' | sed 's/%:/¢/g' |  tr ":+" " " | cut -d " " -f1 |  tr "€" " " | tr "¢" ":" | sort -u > nouns

# Hent alle lemmaer som går til R
grep ";" ../../../src/morphology/stems/nouns.lexc | egrep -v "(^\!|\+Err\/Lex)" | grep ' R '| tr ":+" " " | cut -d " " -f1 |  sort -u > Rnouns
...
grep ";" ../../../src/morphology/stems/nouns.lexc | egrep -v "(^\!|\+Err\/Lex)" | grep ' R '| cut -d ":" -f1 | cut -d "+" -f1 | sed 's/$/+N+Cmp#gåetie+N+Sg+Nom/' | $LOOKUP $generator| cut -f2 | sed 's/gåetie//' > Ranalnouns
